"""Automated downgrade (plan downgrade) flow via Playwright.

Navigates: login -> meus contratos -> acoes -> mudanca de plano (downgrade) ->
...
class DowngradeFlow(PortalFlowBase):
    def __init__(self, email, senha, headless=False, reports_dir=None,
                 on_step=None, config=None):
        super().__init__(email, senha, headless, reports_dir, on_step, "downgrade")
        self.config = config or DowngradeConfig()
        self.resultado_downgrade = None

    def run(self):
        print(f"=== Downgrade Automatizado ===")
        ...
        def steps(page):
            ...
            self._step3_acoes_downgrade(page)
    ...
    def _step3_acoes_downgrade(self, page):
        self._click_acoes_action(page, [
            "Downgrade de plano", "Mudança de plano", "Mudanca de plano",
            "Downgrade", "Mudar plano",
        ], step_num=3)

    def _step4_selecionar_plano(self, page):
        """Select the new (lower value) plan."""
        try:
            ...
            if "vencida" in text.lower() and "replanejamento" in text.lower():
                self._snap(page, 4)
                self._log(4, "Downgrade blocked - overdue parcelas", "failed",
                          "Parcelas vencidas impedem downgrade")
                return

            # Select lower-value plan
            plan_btns = page.locator('[role="dialog"] button:has-text("Selecionar")')
            count = plan_btns.count()

            if count > 0:
                # For downgrade, select the last option (cheapest)
                plan_btns.last.click()
                self._wait_stable(page, 2000)
                print(f"    Selected downgrade plan (last of {count} options)")
            else:
            ...
            self._snap(page, 4)
            self._log(4, "New plan selected (downgrade)", "passed")
        ...
        try:
            ...
            for line in text.split("\n"):
                ...
                if ":" in line:
                    ...
                    val = val.strip()
                    if "R$" in val or "credito" in key.lower():
                        info[key.strip()] = val
            if info:
                print("    Downgrade calculation:")
                for k, v in list(info.items())[:8]:
                    print(f"      {k}: {v}")
            self.resultado_downgrade = info

            self._snap(page, 5)
            summary = ", ".join(details) if details else "defaults"
            self._log(5, f"Installments ({summary})", "passed")
        except Exception as e:
            self._snap(page, 5)
    ...
    def _step6_confirmar(self, page):
        ...
        try:
            ...
            for label in ["Confirmar downgrade", "Confirmar mudança", "Confirmar mudanca",
                          "Confirmar", "Continuar"]:
            ...
            self._snap(page, 6)
            self._log(6, "Downgrade confirmed", "passed")
        except Exception as e:
            self._snap(page, 6)
            self._log(6, "Confirm downgrade", "failed", str(e))
    ...
    def _step7_verificar(self, page):
        ...
        try:
            ...
            success = any(kw in text.lower() for kw in [
                "sucesso", "downgrade realizado", "mudança realizada",
            ...
            self._snap(page, 7)
            self._log(7, "Downgrade complete" if success else "Final state (see screenshot)", "passed")
        except Exception as e:
            self._snap(page, 7)
            self._log(7, "Verify downgrade", "failed", str(e))
