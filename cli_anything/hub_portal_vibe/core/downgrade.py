"""Downgrade (plan downgrade) data queries and simulation."""
...
def list_mudancas_plano(contrato_id=None, turma_id=None):
    """List downgrade records."""
    filters = {"tipo": "DOWNGRADE"}
    if contrato_id:
...
def get_lotes_inferiores(turma_id, plano_atual_id=None):
    """Get available lotes for downgrade (lower value than current)."""
...
def simular_downgrade(contrato_id, valor_plano_atual, valor_plano_novo,
                      num_parcelas=12, percentual_entrada=10, data_referencia=None):
    """Simulate a downgrade calculation.

    Downgrade: new plan is cheaper. Formando may have credit (overpaid).
